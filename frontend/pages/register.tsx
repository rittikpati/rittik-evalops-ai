  ...
  return (
    <div className="min-h-screen bg-[#0F0F0F] text-[#F5F1EB] flex overflow-hidden">
      ...
      >
        ...
        <div className="absolute inset-0 opacity-[0.025]" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9'/%3E%3C/filter%3E%3Crect width='100%' height='100%' filter='url(%23n)'/%3C/svg%3E")` }} />
        <motion.div
          ...
          <div className="space-y-10">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
              ...
              <h1 className="mt-6 font-serif text-[44px] xl:text-[52px] leading-[0.9] tracking-[-0.03em] text-[#F5F1EB]">
                Create your
                <br />
                <span className="font-light italic text-[#C8A96E]">evaluation workspace</span>
                <br />
                in minutes.
              </h1>
              <p className="mt-6 text-[15px] leading-relaxed text-stone-400 max-w-[420px]">
                Join teams evaluating GPT, Claude, Llama and Qwen across accuracy, hallucination, latency and cost — in one refined workspace.
              </p>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12 bg-[#0F0F0F] relative overflow-hidden">
        ...
        >
          ...
          <div className="mb-8">
            <motion.h1 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="font-serif text-[32px] tracking-[-0.02em] leading-none text-[#F5F1EB]">Create workspace</motion.h1>
            <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="mt-2.5 text-[14px] leading-relaxed text-stone-400">Set up your evaluation workspace. Your data stays private and isolated.</motion.p>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-[#1A1A1E] rounded-2xl border border-[#2A2A28] p-7 shadow-[0_16px_48px_rgba(0,0,0,0.3)]">
            <form onSubmit={handleSubmit} className="space-y-5">
              <Input label="Full name" type="text" placeholder="Your name" value={name} onChange={(e) => setName(e.target.value)} error={errors.name} leftIcon={<User className="h-4 w-4" />} autoComplete="name" disabled={isLoading} />
              ...
              <div>
                <Input label="Password" type={showPassword ? "text" : "password"} placeholder="•••••••• (min 8 chars)" value={password} onChange={(e) => setPassword(e.target.value)} error={errors.password} leftIcon={<Lock className="h-4 w-4" />} rightIcon={<button type="button" onClick={() => setShowPassword(!showPassword)} className="text-stone-500 hover:text-[#F5F1EB] transition" aria-label={showPassword ? "Hide" : "Show"}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>} autoComplete="new-password" disabled={isLoading} />
              </div>
              <Input label="Confirm password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} error={errors.confirmPassword} leftIcon={<Lock className="h-4 w-4" />} autoComplete="new-password" disabled={isLoading} />
            ...
            <p className="mt-6 text-center text-sm text-stone-400">
              Already have an account? <Link href="/login" className="font-medium text-[#C8A96E] hover:underline">Sign in</Link>
            </p>
          </motion.div>
